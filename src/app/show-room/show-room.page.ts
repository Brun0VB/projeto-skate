...
export class ShowRoomPage implements OnInit {
  ...
  async showPopUp(produto){
    ...
    const alert = await this.alertController.create({
      header: produto.nome,
      message: `<img src=${this.skateImg[produto.$key]}>${produto.tamanho}<br> ${produto.lixa}<br> ${produto.material}<br> ${produto.modelo}<br> ${produto.marca}<br> ${produto.linha}<br> ${produto.resina}<br><ion-text color="success">Valor: R$${produto.valor}</ion-text>`, 
      buttons: [
        {
          text: 'CANCELAR',
          role: 'cancel',
